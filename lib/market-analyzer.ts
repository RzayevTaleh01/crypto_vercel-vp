...
export class MarketAnalyzer {
  ...
  private pairUpdateInterval: NodeJS.Timeout | null = null
  private database: NeonDatabaseService
  private binance: BinanceTestnetService | null = null
  private latestAnalysis: AnalysisResult[] = []
  ...
  constructor() {
    ...
    try {
      ...
      console.warn("MarketAnalyzer: Binance service not available")
    }
  }
  ...
  async startAnalysis() {
    ...
    this.isRunning = true
    await this.database.addLog("INFO", "Market analysis başladı")
    ...
    this.analysisInterval = setInterval(async () => {
      ...
      try {
        ...
        await this.database.addLog("ERROR", "Analysis xətası", { error: error.message })
      }
    }, 30000) // 30 seconds

    // Start pair update loop
    this.pairUpdateInterval = setInterval(async () => {
      if (!this.isRunning) return
      ...
      try {
        ...
        await this.database.addLog("ERROR", "Pair update xətası", { error: error.message })
      }
    }, 300000) // 5 minutes
  ...
  private async performAnalysis() {
    if (!this.binance) {
      await this.database.addLog("WARNING", "Binance service yoxdur, analysis keçilir")
      return
    }
    ...
    for (const symbol of this.symbols) {
      try {
        ...
        const signals = this.generateSignals(marketData, technicalIndicators)
        const confidence = this.calculateConfidence(marketData, technicalIndicators, signals)
        const reasons = this.generateReasons(marketData, technicalIndicators, signals)
        ...
        await this.database.addLog("ERROR", `${symbol} analysis xətası`, { error: error.message })
      }
    }

    this.latestAnalysis = results
    await this.database.addLog("DEBUG", "Analysis tamamlandı", { pairCount: results.length })
  }
  ...
  private generateSignals(marketData: MarketData, indicators: TechnicalIndicators) {
    const rsiSignal = 
      indicators.rsi < 30 ? "BUY" : 
      indicators.rsi > 70 ? "SELL" : "NEUTRAL"

    const smaSignal = 
      marketData.price > indicators.sma20 && indicators.sma20 > indicators.sma50 ? "BUY" :
      marketData.price < indicators.sma20 && indicators.sma20 < indicators.sma50 ? "SELL" : "NEUTRAL"

    const macdSignal = "NEUTRAL" // Simplified

    // Overall signal logic
    const buySignals = [rsiSignal, smaSignal, macdSignal].filter(s => s === "BUY").length
    const sellSignals = [rsiSignal, smaSignal, macdSignal].filter(s => s === "SELL").length
    ...
    return {
      overall: overall as "BUY" | "SELL" | "HOLD",
      rsi: rsiSignal as "BUY" | "SELL" | "NEUTRAL",
      macd: macdSignal as "BUY" | "SELL" | "NEUTRAL",
      sma: smaSignal as "BUY" | "SELL" | "NEUTRAL",
    }
  }

  private calculateConfidence(marketData: MarketData, indicators: TechnicalIndicators, signals: any): number {
    let confidence = 50

    // Volume factor
    if (marketData.volume > 1000000) confidence += 10

    // RSI strength
    if (indicators.rsi < 25 || indicators.rsi > 75) confidence += 15
    if (indicators.rsi < 35 || indicators.rsi > 65) confidence += 10

    // Price change
    if (Math.abs(marketData.priceChangePercent) > 2) confidence += 10

    // Signal alignment
    const signalCount = [signals.rsi, signals.sma].filter(s => s === signals.overall).length
    confidence += signalCount * 10

    return Math.min(100, Math.max(0, confidence))
  }

  private generateReasons(marketData: MarketData, indicators: TechnicalIndicators, signals: any): string[] {
    const reasons: string[] = []

    if (signals.rsi === "BUY") reasons.push(`RSI oversold (${indicators.rsi.toFixed(1)})`)
    if (signals.rsi === "SELL") reasons.push(`RSI overbought (${indicators.rsi.toFixed(1)})`)
    
    if (signals.sma === "BUY") reasons.push("Price above SMA20")
    if (signals.sma === "SELL") reasons.push("Price below SMA20")

    if (marketData.volume > 1000000) reasons.push("High volume")
    if (Math.abs(marketData.priceChangePercent) > 3) reasons.push("Strong price movement")
  ...
  private async updateTradingPairs() {
    ...
    try {
      ...
      const allTickers = await this.binance.getAllTickers()
      const usdtPairs = allTickers
        .filter(t => t.symbol.endsWith("USDT"))
        .filter(t => Number(t.volume) > 500000)
        .sort((a, b) => Number(b.volume) - Number(a.volume))
        .slice(0, 10)
        .map(t => t.symbol)

      this.symbols = usdtPairs
      await this.database.addLog("INFO", "Trading pairs yeniləndi", { pairs: usdtPairs })
    } catch (error: any) {
      await this.database.addLog("ERROR", "Pair update xətası", { error: error.message })
    }
  }
